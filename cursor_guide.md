...
- **语言**：设置 IDE 显示语言，可在 规则文档 中进一步调整 AI 语言行为。
...
此外，Cursor IDE 在界面中集成了 AI 功能入口，如聊天窗口和 Tab 补全提示。
...
- **生成命令**：输入自然语言描述，AI 生成对应的终端命令。
- **解释输出**：选择终端输出，请求 AI 解释其含义。
...
- **AI 行为**：通过 .cursorrules 文件定义 AI 的响应规则（详见 规则文档）。
...
- **提交**：通过源控制面板提交更改，AI 可生成提交消息（点击“魔法棒”图标）。
- **分支管理**：创建、切换或合并分支。
- **冲突解决**：使用内置工具解决合并冲突。
...
Cursor 官方文档 提供全面的指南，涵盖安装、功能使用和故障排除。

### 7.2 社区论坛

Cursor IDE 拥有活跃的社区论坛，用户可以：
...
- GitHub - Cursor AI 代码编辑器
- DataCamp - Cursor AI 指南与示例
- Builder.io - Cursor 开发者介绍
- Builder.io - Cursor 使用技巧
- Medium - Cursor AI 设置指南
- Wikipedia - Cursor 代码编辑器